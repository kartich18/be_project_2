...
def validate_transaction_payload(data: dict) -> tuple[bool, str]:
    ...
    if not isinstance(data["sender"], str) or not data["sender"].strip():
        return False, "Sender must be a non-empty string"
    if not isinstance(data["receiver"], str) or not data["receiver"].strip():
        return False, "Receiver must be a non-empty string"
    if data["sender"].strip() == data["receiver"].strip():
