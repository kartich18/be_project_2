...
class TestPostToMetricsFlow:
    ...
    def test_single_transaction_reflected_in_metrics(self, app, client):
        ...
        assert tx_data["classical"]["status"] == "success"
    ...
    def test_multiple_transactions_aggregation(self, app, client):
    ...
    def test_method_filter_after_transactions(self, app, client):
...
class TestBenchmarkCoexistence:
    ...
    def test_transaction_then_benchmark(self, client):
        """POST a transaction, then run a small benchmark — both succeed."""
        # Transaction
        resp1 = client.post("/api/transaction", json=VALID_PAYLOAD)
        assert resp1.status_code == 201

        # Benchmark (small iteration count for speed)
        resp2 = client.get("/api/benchmark?iterations=2")
...
class TestDashboardRoute:
    ...
    def test_root_returns_html(self, client):
        ...
        resp = client.get("/")
        assert resp.status_code == 200
        assert b"<" in resp.data  # Basic HTML check
...
class TestEdgeCases:
    ...
    def test_large_sender_name(self, client):
        """Transaction with a very long sender name should still succeed."""
        resp = client.post("/api/transaction", json={
            "amount": 500.0,
            "sender": "A" * 120,  # Close to 128-char limit
            "receiver": "Bob",
        })
        assert resp.status_code == 201
        data = resp.get_json()
...
class TestRapidFireRequests:
    ...
    def test_rapid_fire_transactions(self, app, client):
        """5 rapid-fire POSTs should all succeed and create correct DB state."""
        responses = []
        for i in range(5):
            resp = client.post("/api/transaction", json={
                "amount": 100 * (i + 1),
                "sender": f"RapidSender{i}",
                "receiver": f"RapidReceiver{i}",
            })
...
class TestPQCFallback:
    ...
    def test_transaction_succeeds_without_liboqs(self, client):
    ...
    def test_both_methods_succeed_with_liboqs(self, client):
        ...
        assert data["classical"]["status"] == "success"
        assert "pqc" in data
        assert data["pqc"]["status"] == "success"
